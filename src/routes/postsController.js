...
const rows = await listPosts({ limit: Number(req.query.limit || 20), offset: Number(req.query.offset || 0) });
res.json(rows);
}
...
const post = /^[0-9]+$/.test(idOrSlug) ? await getPostById(Number(idOrSlug)) : await getPostBySlug(idOrSlug);
...
const created = await createPost({ title, slug, content_html: safeHtml, content_src: content, status });
...
const { title, content, status = existing.status, sourceType = 'markdown' } = req.body;
const slug = slugify(req.body.slug || title || existing.title);
const html = sourceType === 'markdown' ? marked.parse(content ?? existing.content_src ?? '') : (content ?? existing.content_html);
const safeHtml = sanitizeHtml(html, SANITIZE_OPTS);
